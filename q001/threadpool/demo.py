...
Work model:
    ...
    Main threads will continuing adding tasks (task parameters) to task jobqueue.
    Many outer workers(in many threads and many processes) would read tasks from jobqueue one by one and work them out,
        then put the result(if we have) into the product jobqueue.
    An handler thread in main process will read the products in the product jobqueue(if we have),
...
def worker(index, t=None):
    ...
    # worker's return value will be added to product jobqueue, waiting handler to handle
...
def collector(meta, result):
    ...
    It is running in single thread in the main process,
        if you want to have multi-threads handler, you can simply pass it's arg(s) to another working jobqueue
