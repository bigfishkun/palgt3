...
class KdataDbCache():
    ...
    def get_k_data(self, index, start, end, code,*args, **kwargs):
        """
        ...
        """
        if 'index' in kwargs and index == True:
            return k_index_dao.KdataDbCache().get_k_data(*args,**kwargs)
        ...
        if df is None or df['date'].values[-1]< end:  #可能需要更新數據，更新數據的时候注意重新判定end的日期，因为有可能不是交易日
            ...
            #self._update_cache()
            # data = ts.get_k_data(*args,**kwargs)  #新版的get_k_data方法仍需要指明是否为index
            data = ts.get_k_data(code=code, index = False, start = start, end = end)
